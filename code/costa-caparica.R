...
schools_c1$ciclo_school = grepl(
  "Escola Básica n.º 2 da Costa da Caparica, Almada",
  schools_c1$ESCOLA
)
...
for (plan in plans) {
  ...
  if (file.exists(routes_plan_location)) {
    ...
    saveRDS(
      routes_plan,
      paste0("../internal/routes-", plan, "-costa-da-caparica.Rds")
    )
  }
...
for (plan in plans) {
  ...
  route_summaries = routes_plan_all |>
    group_by(route_number) |>
    summarise(
      trips_modelled = mean(trips_modelled),
      length = mean(length),
...
weightedMedian(
  ...
  route_summaries_all_quiet$trips_modelled
)
# [1] 814.0006
weightedMedian(
  route_summaries_all_fast$length,
  route_summaries_all_fast$trips_modelled
)
# [1] 785.9941
...
for (plan in plans) {
  ...
  rnet_plan = rnet_plan_raw |>
    transmute(
      trips_modelled = trips_modelled_sum,
...
for (plan in plans) {
  ...
  route_summaries = routes_plan_pct |>
    rename(OBJECTID = O) |>
    group_by(OBJECTID, route_number, trips_modelled, bicycle_godutch, length) |>
    summarise() |>
    ungroup()
...
tm_shape(zone_centroids) +
  tm_bubbles("N_INDIVIDUOS_0_14", alpha = 0.3) +
  tm_shape(ciclo_school) +
...
# Scheduled stops
actual_route_geo = sf::read_sf(
  "../paper/route-data/costadacaparica.geojson",
  quiet = TRUE
)

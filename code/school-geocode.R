...
# AML
AMLgeo = readRDS(url("https://github.com/U-Shift/biclar/releases/download/0.0.1/MUNICIPIOSgeo.Rds"))
AMLgeo$Concelho[AMLgeo$Concelho == "Setubal"] = "Setúbal"
Concelhos_list = unique(AMLgeo$Concelho)

# TML school coordinates
schools_official_coors = read.csv("https://github.com/carrismetropolitana/datasets/raw/latest/facilities/schools/schools.csv")
schools_official_coors = st_as_sf(schools_official_coors, coords = c("lon", "lat"), crs = 4326)
...
schools_dgeec = readxl::read_xlsx("data_original/DGEEC_AlunosMatriculados_Continente2122.xlsx")
...
schools_dgeec_totals = schools_dgeec |> 
  ...
  group_by(`CÓDICO DGEEC ENTIDADE`, ENTIDADE, `CÓDIGO DGEEC ESCOLA`, ESCOLA, `CÓDIGO DGEEC AGRUPAMENTO`, AGRUPAMENTO,
           `CÓDIGO DGEEC ESCOLA SEDE`, `ESCOLA SEDE`, MUNICÍPIO, `CICLO DE ESTUDOS`) |> 
...
schools_dgeec_escola_coors = schools_dgeec_escola |> 
  left_join(schools_official_coors |> select(name, municipality_name, geometry),
            by = c("ESCOLA_nome" = "name", "MUNICÍPIO" = "municipality_name")) |> 
  st_as_sf()
  

schools_dgeec_escola_coors_OK = schools_dgeec_escola_coors |> filter(!st_is_empty(geometry)) # 93%
schools_dgeec_escola_coors_MISSING = schools_dgeec_escola_coors |> filter(st_is_empty(geometry)) |> filter(!is.na(ESCOLA))
...
schools_dgeec_escola_coors_MISSING$ESCOLA_nome[schools_dgeec_escola_coors_MISSING$ESCOLA_nome == "Colégio Art & Manha"] = "Colégio Art e Manha"
schools_dgeec_escola_coors_MISSING$address = paste0(schools_dgeec_escola_coors_MISSING$ESCOLA_nome, ", ", schools_dgeec_escola_coors_MISSING$MUNICÍPIO, ", Portugal")
...
schools_dgeec_escola_coors = rbind(schools_dgeec_escola_coors_OK,
...
schools_dgeec_geo = schools_dgeec_totals |> 
  ...
  left_join(schools_dgeec_escola_coors |> 
...
SCHOOLS_year = schools_dgeec |> 
  filter(`NÍVEL DE  ENSINO` %in% c("Ensino básico", "Ensino secundário") ) |>
  group_by(`CÓDIGO DGEEC ESCOLA`, ESCOLA, MUNICÍPIO, NATUREZA, `CICLO DE ESTUDOS`, `ANO DE ESCOLARIDADE`) |> 
  summarise(Alunos = sum(`NÚMERO DE ALUNOS MATRICULADOS`)) |> 
  ungroup()

names(SCHOOLS_year) = c("DGEEC_id", "ESCOLA", "MUNICIPIO", "PUBLIC_PRIVATE", "CICLO_ESTUDOS", 
  "ANO_ESCOLARIDADE", "STUDENTS")

# change CICLO_ESTUDOS to "Secondary" if ANO_ESCOLARIDADE is 10, 11 or 12
SCHOOLS_year$CICLO_ESTUDOS[SCHOOLS_year$ANO_ESCOLARIDADE %in% c("10.º Ano", "11.º Ano", "12.º Ano")] = "Secondary"

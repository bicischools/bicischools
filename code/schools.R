...
schools_both = inner_join(
  schools_year,
  schools_geo,
  by = c("DGEEC_id" = "CÓDIGO DGEEC ESCOLA")
)
...
no_ciclo = no_ciclo |>
  mutate(
    ...
    basic_and_secondary = case_when(
      basica & secundaria ~ "yes",
      TRUE ~ "unclear"
    ),
...
no_ciclo = no_ciclo |>
  mutate(
    ciclo_1_students = case_when(
      basic_only == "yes" ~ round(STUDENTS / 9 * 4),
      TRUE ~ 0
    ),
    ciclo_2_students = case_when(
      basic_only == "yes" ~ round(STUDENTS / 9 * 2),
      basic_and_secondary == "yes" ~ round(STUDENTS / 8 * 2),
      TRUE ~ 0
    ),
    ciclo_3_students = case_when(
      basic_only == "yes" ~ round(STUDENTS / 9 * 3),
...
##################################################
lisbon_c1 = by_ciclo |>
  filter(MUNICIPIO == "Lisboa", CICLO_ESTUDOS == "1.º Ciclo")
lisbon_c2 = by_ciclo |>
  filter(MUNICIPIO == "Lisboa", CICLO_ESTUDOS == "2.º Ciclo")
lisbon_c3 = by_ciclo |>
  filter(MUNICIPIO == "Lisboa", CICLO_ESTUDOS == "3.º Ciclo")

tm_shape(bgri_lisbon) +
  tm_polygons("N_INDIVIDUOS_0_14") +
  tm_shape(lisbon_c1) +
  tm_dots(size = "STUDENTS")
tm_shape(bgri_lisbon) +
  tm_polygons("N_INDIVIDUOS_0_14") +
  tm_shape(lisbon_c2) +
  tm_dots(size = "STUDENTS")
...
########################
c1_public = by_ciclo |>
  filter(CICLO_ESTUDOS == "1.º Ciclo", PUBLIC_PRIVATE == "Público")
c2_public = by_ciclo |>
  filter(CICLO_ESTUDOS == "2.º Ciclo", PUBLIC_PRIVATE == "Público")
c3_public = by_ciclo |>
  filter(CICLO_ESTUDOS == "3.º Ciclo", PUBLIC_PRIVATE == "Público")

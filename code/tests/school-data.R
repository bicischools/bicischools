...
library(readxl)
dgeec_schools = read_xlsx(glue::glue(
  "{data_dir}/DGEEC_AlunosMatriculados_Continente2122.xlsx"
))
...
school_geom_filtered = school_geom |>
  filter(municipality_name == "Lisboa", university == 0, pre_school == 0)

# Join with dgeec data based on school names
schools_joined = inner_join(
  dgeec_totals,
  school_geom,
  by = c("ESCOLA" = "name")
) # only 7 matches

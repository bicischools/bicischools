...
The input datasets are illustrated in the figure below (these are `home_zones` and `schools_c1` that are provided with the package):
...
There are more resident children than school students, because we are only looking at schools from Ciclo 1, which covers four school years. 
Also, younger children won't attend school and others will go to private schools. 
...
# od_from_si_full = simodels::si_to_od(home_zones, schools_c1)
...
```{r}
#| include: false
od_observed = readRDS("data/home_zones.Rds")

...
The input datasets are illustrated in the figure below (these are `zone_counts` and `school` that are provided with the pacage):

```{r}
#| label: inputs
zone_counts = readRDS("data/zone_counts.Rds")
school = readRDS("../internal/Bicischools_school_sample.Rds")
sf::st_crs(school) = 4326
school = school |> 
  mutate(
    id = 111,
    n_pupils = 169
...
zone_overestimate_factor = 
  (sum(zone_counts$N_INDIVIDUOS_0_14) /
    sum(school$n_pupils))
zone_overestimate_factor
```

As we only have one school, the total number of children resident in the zones dataset is many times larger than the number of students going to this single school.
...
#| echo: true
sum(zone_counts$pupils_estimated)
sum(school$n_pupils)
```
...
od_from_si_full = simodels::si_to_od(zone_counts, school)
od_from_si = simodels::si_to_od(zone_counts, school, max_dist = max_dist)
```
...
```{r}
#| include: false
od_observed = readRDS("data/zone_counts.Rds")
od_observed = od_observed |> 
  mutate(D = 111) |> 
  select(OBJECTID, D)
...
#| label: adjust-output
#| echo: true
interaction_overestimate_factor = sum(school$n_pupils) / sum(od_res$interaction)
od_res = od_res |>
  dplyr::mutate(
...
sum(res_constrained$interaction) == sum(school$n_pupils)
